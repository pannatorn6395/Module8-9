...
def getContours(img, edges, iters=10):
    ...
    cnt = simplifyContours(contours)

    return np.array(cnt), hierarchy[0]
...
def simplifyContours(contours):
    ...
    for i in range(len(contours)):
        ...
        # contours[i] = cv2.approxPolyDP(contours[i], 4.5, True)
        cnt.append(cv2.approxPolyDP(
    ...
    for i in range(0, len(matrixOfPoints)):
        for j in range(0, len(matrixOfPoints[i])):
            ...
            if i*9+j == 0 or i*9+j == 8 or i*9+j == 72 or  i*9+j == 80:
                cv2.putText(
                img, f"{(math.floor(matrixOfPoints[i, j, 0]))}", (math.floor(matrixOfPoints[i, j, 0]), math.floor(matrixOfPoints[i, j, 1])), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 1)
